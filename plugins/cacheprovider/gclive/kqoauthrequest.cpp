...
QString KQOAuthRequestPrivate::oauthSignature()  {
    ...
    QString secret = QString(QUrl::toPercentEncoding(oauthConsumerSecretKey)) + "&" + QString(oauthTokenSecret);
...
QByteArray KQOAuthRequestPrivate::requestBaseString() {
    ...
    if (debugOutput) {
        ...
        qDebug() << QUrl::fromPercentEncoding(baseString) << "\n";
    }
...
QByteArray KQOAuthRequestPrivate::encodedParamaterList(const QList< QPair<QString, QString> > &parameters) {
	...
	//                  + QUrl::toPercentEncoding(parameter.second, "", "+/")  // Parameter value
        resultList.append( parameter.first     // Parameter key
                           + "="
                           + parameter.second  // Parameter value
                          );
        if (debugOutput) {
            qDebug() << " * "
                     << parameter.first
...
void KQOAuthRequest::setCallbackUrl(const QUrl &callbackUrl) {
    ...
    d->oauthCallbackUrl = QUrl::toPercentEncoding(callbackUrl.toString());
}
...
void KQOAuthRequest::setVerifier(const QString &verifier) {
    ...
    //    if(d->oauthVerifier.endsWith("=")) {
    //      qDebug() << "verifier ENDSWIDTH =!!!!!!!";
    //      d->oauthVerifier.chop(1);      
    //    }
}
