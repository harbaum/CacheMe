...
bool KQOAuthManagerPrivate::setSuccessfulRequestToken(const QMultiMap<QString, QString> &request) {
    ...
    if (hasTemporaryToken) {
      requestToken = QString(request.value("oauth_token")).toLocal8Bit();
      requestTokenSecret =  QString(request.value("oauth_token_secret")).toLocal8Bit();
    }
...
void KQOAuthManager::getUserAuthorization(QUrl authorizationEndpoint) {
    ...
    openWebPageUrl.addEncodedQueryItem(QUrl::toPercentEncoding(tokenParam.first), tokenParam.second.toAscii());

    // Open the user's default browser to the resource authorization page provided
    // by the service.
    qDebug() << __FUNCTION__ << openWebPageUrl.toString();

    emit openUrl(openWebPageUrl);
}
...
void KQOAuthManager::getUserAccessTokens(QUrl accessTokenEndpoint) {
    ...
    d->opaqueRequest->setEnableDebugOutput(true);
...
void KQOAuthManager::onRequestReplyReceived( QNetworkReply *reply ) {
    ...
    */
    ...
    qDebug() << __FUNCTION__ << networkReply;
