...
QGeoRouteReplyCm::QGeoRouteReplyCm(const QGeoRouteRequest &request, QNetworkReply *reply, QObject *parent)
...
QGeoRouteReplyCm::~QGeoRouteReplyCm()
{
}

void QGeoRouteReplyCm::abort()
{
    if (!m_reply)
        return;
...
void QGeoRouteReplyCm::networkFinished()
{
    if (!m_reply)
        return;
    ...
    QGeoRouteJsonParser parser(request());
...
void QGeoRouteReplyCm::networkError(QNetworkReply::NetworkError error)
{
    Q_UNUSED(error)
